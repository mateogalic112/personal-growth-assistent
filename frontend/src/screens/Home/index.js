...
const Home = () => {
	...
	const { loading, error, goals } = useSelector((state) => state.goalList);

	const goalsByDate = goals
		? goals.filter(
				(goal) =>
					goal.date.split('T')[0] ===
	...
	const completedGoals = goalsByDate
		? goalsByDate.filter((goal) => goal.isCompleted)
		: [];
			...
			<Container>
							...
							<b>
								{parseFloat(
									(completedGoals?.length /
										goalsByDate?.length || 0) * 100
								).toFixed(2)}
								%
							</b>{' '}
							of your goals {dateOrStringToday(startDate)}!
						</p>
					</Content>
				...
				<GoalList goals={goalsByDate} date={startDate}>
					...
					<Stats
						...
						leftGoalsCount={
							goalsByDate?.length - completedGoals?.length
						}
